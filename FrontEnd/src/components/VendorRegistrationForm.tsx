...
export const VendorRegistrationForm: React.FC<VendorRegistrationFormProps> = ({ onRegistered }) => {
  // Form state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Notify parent page to refresh vendor list (if provided)
    onRegistered?.()
      // Reset form
      setFormData({
        userId: '',
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden p-6 md:p-8">
      ...
      {/* vendor list removed from form; moved to its own page */}
    </div>
  )
}
